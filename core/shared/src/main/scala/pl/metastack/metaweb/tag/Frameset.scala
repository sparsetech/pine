 ...
 */
case class Frameset(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[Node] = Seq.empty) extends HTMLTag {
  override type T = Frameset
  override def tagName = "frameset"
  override def copy(attributes: Predef.Map[String, Any] = attributes, children: Seq[Node] = children): Frameset = Frameset(attributes, children)
  /**
  ...
  def cols(value: String): Frameset = copy(attributes = attributes + ("cols" -> value))
  /**
  ...
  def rows(value: String): Frameset = copy(attributes = attributes + ("rows" -> value))
}

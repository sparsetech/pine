 ...
 */
case class Isindex(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[Node] = Seq.empty) extends HTMLTag {
  override type T = Isindex
  override def tagName = "isindex"
  override def copy(attributes: Predef.Map[String, Any] = attributes, children: Seq[Node] = children): Isindex = Isindex(attributes, children)
  /**
  ...
  def prompt(value: String): Isindex = copy(attributes = attributes + ("prompt" -> value))
  /**
  ...
  def action(value: String): Isindex = copy(attributes = attributes + ("action" -> value))
}

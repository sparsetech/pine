...
object InlineHtml {
  ...
  def iter(c: Context)(node: Node,
    ...
    val nodeType = c.mirror.staticClass("pl.metastack.metaweb.Node")
       .toType
      ...
      case Text(text) =>
        ...
        parts.map { v =>
          if (!v.startsWith("${") || !v.endsWith("}"))
            c.Expr(q"Seq(pl.metastack.metaweb.Text($v))")
          else {
            ...
            args(index) match {
              ...
              case n if n.tree.tpe =:= integerType ||
                ...
                c.Expr(q"Seq(pl.metastack.metaweb.Text($n.toString))")
              case n if n.tree.tpe =:= stringType =>
                c.Expr(q"Seq(pl.metastack.metaweb.Text($n))")
              case n =>

 ...
 */
case class Datalist(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[Node] = Seq.empty) extends HTMLTag {
  override type T = Datalist
  override def tagName = "datalist"
  override def copy(attributes: Predef.Map[String, Any] = attributes, children: Seq[Node] = children): Datalist = Datalist(attributes, children)
}

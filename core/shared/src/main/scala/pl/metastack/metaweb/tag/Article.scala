 ...
 */
case class Article(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[Node] = Seq.empty) extends HTMLTag {
  override type T = Article
  override def tagName = "article"
  override def copy(attributes: Predef.Map[String, Any] = attributes, children: Seq[Node] = children): Article = Article(attributes, children)
}

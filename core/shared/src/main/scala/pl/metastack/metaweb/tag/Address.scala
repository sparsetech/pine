 ...
 */
case class Address(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[Node] = Seq.empty) extends HTMLTag {
  override type T = Address
  override def tagName = "address"
  override def copy(attributes: Predef.Map[String, Any] = attributes, children: Seq[Node] = children): Address = Address(attributes, children)
}

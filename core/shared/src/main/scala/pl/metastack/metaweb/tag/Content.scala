 ...
 */
case class Content(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[Node] = Seq.empty) extends HTMLTag {
  override type T = Content
  override def tagName = "content"
  override def copy(attributes: Predef.Map[String, Any] = attributes, children: Seq[Node] = children): Content = Content(attributes, children)
}

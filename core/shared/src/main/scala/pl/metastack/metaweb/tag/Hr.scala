 ...
 */
case class Hr(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[Node] = Seq.empty) extends HTMLTag {
  override type T = Hr
  override def tagName = "hr"
  override def copy(attributes: Predef.Map[String, Any] = attributes, children: Seq[Node] = children): Hr = Hr(attributes, children)
  /**
...
<code>left</code>.
  ...
  def align(value: String): Hr = copy(attributes = attributes + ("align" -> value))
  /**
   * Sets the color of the rule through color name or hexadecimal value.
   */
  def color: scala.Option[String] = attributes.get("color").asInstanceOf[scala.Option[String]]
  def color(value: String): Hr = copy(attributes = attributes + ("color" -> value))
  /**
   * Sets the rule to have no shading.
   */
  def noshade: Boolean = attributes.contains("noshade")
  def noshade(value: Boolean): Hr = (if (value) copy(attributes = attributes + ("noshade" -> "")) else copy(attributes = attributes - "noshade"))
  /**
   * Sets the height, in pixels, of the rule.
   */
  def size: scala.Option[String] = attributes.get("size").asInstanceOf[scala.Option[String]]
  def size(value: String): Hr = copy(attributes = attributes + ("size" -> value))
  /**
   * Sets the length of the rule on the page through a pixel or percentage value.
   */
  def width: scala.Option[String] = attributes.get("width").asInstanceOf[scala.Option[String]]
  def width(value: String): Hr = copy(attributes = attributes + ("width" -> value))
}

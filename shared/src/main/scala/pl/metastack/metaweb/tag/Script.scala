 ...
 */
case class Script(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[tree.Node] = Seq.empty) extends HTMLTag[Script] {
...
<strong>src</strong> attribute).
  ...
  def async(value: String): Script = copy(attributes = attributes + ("async" -> value.toString)).asInstanceOf[Script]
  /**
...
<code>src</code> attribute specified should not have a script embedded within its tags.
  ...
  def src(value: String): Script = copy(attributes = attributes + ("src" -> value.toString)).asInstanceOf[Script]
  /**
...
<code>type=application/javascript;version=1.8</code>. Beware, however, that&nbsp;as this is a non-standard feature, this will most likely break support for&nbsp;other browsers, in particular Chromium-based browsers.
  ...
  def `type`(value: String): Script = copy(attributes = attributes + ("type" -> value.toString)).asInstanceOf[Script]
  /**
   * Like the 
...
<code>type</code> attribute should be used instead.
  ...
  def language(value: String): Script = copy(attributes = attributes + ("language" -> value.toString)).asInstanceOf[Script]
  /**
...
<code>defer</code> attribute is set.
  ...
  def defer(value: String): Script = copy(attributes = attributes + ("defer" -> value.toString)).asInstanceOf[Script]
  /**
   * Normal 
...
<a href="http://lists.whatwg.org/pipermail/whatwg-whatwg.org/2012-February/034969.html" class="external">WHATWG mailing list</a>.
  ...
  def crossorigin(value: String): Script = copy(attributes = attributes + ("crossorigin" -> value.toString)).asInstanceOf[Script]
  /**
  ...
  def integrity(value: String): Script = copy(attributes = attributes + ("integrity" -> value.toString)).asInstanceOf[Script]
}

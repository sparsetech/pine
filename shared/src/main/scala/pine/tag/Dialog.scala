 ...
 */
case class Dialog(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[Node] = Seq.empty) extends HTMLTag {
  override type T = Dialog
  override def tagName = "dialog"
  override def copy(attributes: Predef.Map[String, Any] = attributes, children: Seq[Node] = children): Dialog = Dialog(attributes, children)
  /**
  ...
  def open(value: Boolean): Dialog = (if (value) copy(attributes = attributes + ("open" -> "")) else copy(attributes = attributes - "open"))
}

...
<code>track</code> element per media element.
  ...
  def default(value: String): Track = copy(attributes = attributes + ("default" -> value))
  /**
...
<ul> 
 ...
 <li><code>metadata</code> 
  ...
  def kind(value: String): Track = copy(attributes = attributes + ("kind" -> value))
  /**
   * A user-readable title of the text track which is used by the browser when listing available text tracks.
   */
  def label: scala.Option[String] = attributes.get("label").asInstanceOf[scala.Option[String]]
  def label(value: String): Track = copy(attributes = attributes + ("label" -> value))
  /**
...
<code>.vtt</code> file). Must be a valid URL. This attribute must be defined.
  ...
  def src(value: String): Track = copy(attributes = attributes + ("src" -> value))
  /**
...
<code>srclang</code> must be defined.
  ...
  def srclang(value: String): Track = copy(attributes = attributes + ("srclang" -> value))
}

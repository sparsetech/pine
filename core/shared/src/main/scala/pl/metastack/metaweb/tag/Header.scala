 ...
 */
case class Header(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[Node] = Seq.empty) extends HTMLTag {
  override type T = Header
  override def tagName = "header"
  override def copy(attributes: Predef.Map[String, Any] = attributes, children: Seq[Node] = children): Header = Header(attributes, children)
}

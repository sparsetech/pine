 ...
 */
case class Dialog(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[tree.Node] = Seq.empty) extends tree.Tag with HTMLTag {
   ...
   */
  def open: scala.Option[Boolean] = attributes.get("open").asInstanceOf[scala.Option[Boolean]]
}

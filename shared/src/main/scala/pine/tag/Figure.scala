 ...
 */
case class Figure(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[Node] = Seq.empty) extends HTMLTag {
  override type T = Figure
  override def tagName = "figure"
  override def copy(attributes: Predef.Map[String, Any] = attributes, children: Seq[Node] = children): Figure = Figure(attributes, children)
}

 ...
 */
case class Main(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[Node] = Seq.empty) extends HTMLTag {
  override type T = Main
  override def tagName = "main"
  override def copy(attributes: Predef.Map[String, Any] = attributes, children: Seq[Node] = children): Main = Main(attributes, children)
}

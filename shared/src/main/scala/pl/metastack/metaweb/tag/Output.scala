 ...
 */
case class Output(attributes: Predef.Map[String, Any] = Predef.Map.empty, children: Seq[tree.Node] = Seq.empty) extends HTMLTag[Output] {
  ...
  def `for`(value: String): Output = copy(attributes = attributes + ("for" -> value.toString)).asInstanceOf[Output]
  /**
  ...
  def name(value: String): Output = copy(attributes = attributes + ("name" -> value.toString)).asInstanceOf[Output]
}
